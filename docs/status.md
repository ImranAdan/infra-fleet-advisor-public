...
| Application contract | The fleet's app contracts (`k8s/fleet-app/fleet-app.yaml` and each `k8s/applications/<name>/fleet-app.yaml`) and a whole-word search of every tracked platform file (`k8s/` outside the apps' own directories, `scripts/`, `policies/`, `platform/`, `fleet`) for any app name, and `${APP_NAME}`-derived names on every Canary, HPA and NetworkPolicy in the applications namespace (M-004) | Divergence-only: a script could name an app no contract declares. Per-app AWS resources, workflows and Terraform are outside the position. An unreadable or excluded contract leaves the verdict unknown unless readable evidence shows coupling; a file it cannot read or a scan beyond 1000 files makes coverage partial; a checkout without `k8s/applications/` yields no evidence |
